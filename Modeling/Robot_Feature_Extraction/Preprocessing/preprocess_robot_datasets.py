...
class PreprocessRobotData():
    ...
    def load_dataset(self):
        filepath = utilities.find("*.npy",self.cst["processed_data_path"])
        for file in filepath:
            if "after_processing_" in file:
                with open(file, 'rb') as f: 

                    if "train" in file:                
                        self.output_dict['train'] = np.load(f)

                    if "val" in file:                
                        self.output_dict['test'] = np.load(f)

                    if "test" in file:                
                        self.output_dict['val'] = np.load(f)
    ...
    def preprocess_data(self):
        ...
        for name,value in self.datasets.items():
            ...
            self.aug_labels.append([name]*np.shape(self.datasets_matrix[name])[0])
          
        self.aug_labels = np.concatenate((self.aug_labels[0],self.aug_labels[1],self.aug_labels[2]))
    ...
    def data_augmentation(self):
        ...
        new_signals_jit = augmentation.jitter(self.output_matrix, sigma=0.002)
        new_signals_mw = augmentation.magnitude_warp(self.output_matrix, sigma=0.8, knot=6)
        new_signals_ww = augmentation.window_warp(self.output_matrix, window_ratio=0.2) 

        new_signals_rw = augmentation.random_guided_warp(self.output_matrix, self.aug_labels.flatten(), 
                                                            slope_constraint='asymmetric')
        self.output_matrix = np.concatenate((self.output_matrix,new_signals_jit, new_signals_mw,
                                                new_signals_ww, new_signals_rw),axis=0)               
    ...
    def start_preprocessing(self, tag=None):
        ...
        if not self.cst["load_from_path"]:
            self.load_files()
