...
from .model_VAE_NLoss_Last_Hidden import NVAE
...
class CustomDatasetExpress(Dataset):
    def __init__(self, input_data, laban_data):
        self.data = input_data
        self.laban_data = laban_data

    def __len__(self):
        if len(self.data) == len(self.laban_data):
            return len(self.data)
        else:
            return 0
    ...
    def __getitem__(self, idx):
        label = self.data[idx]
        qualitie = self.laban_data[idx]
        return label, qualitie 
...
class CustomDataset_Hum_Style_Neutral(Dataset):
    ...
    def __getitem__(self, idx):
        label1 = self.data[idx]
        participant = self.participants_tags[idx]
        neutral_label1 = self.neutral_data[participant]

        return label1, neutral_label1
...
class ModelingProc():
    ...
    def set_input_data(self, input_data, expressive=False):
        if not expressive:
            ...
            self.dataset_tags = input_data[1]
            ...
            self.data = signals_input_data
            self.raw_data = copy.deepcopy(signals_input_data)

        print(self.raw_data.keys())
        self.set_batches(expressive=expressive)
        print('------loading data done')
    
    def test_corr_dataloader_raw(self, key, expressive=False):
        correlationCoeffArr = []
        aux=0
    ...
    def set_batches(self, expressive=False):
        # Create data loaders.
        g = torch.Generator()
        g.manual_seed(0)
        for key,value in self.data.items():
            ...
            if not expressive:
                ...
                value_qualities = self.qualities_input_data[key]
                if not isinstance(value, torch.Tensor) or not isinstance(value_qualities, torch.Tensor):
                    value = torch.tensor(value)
                    value_qualities = torch.tensor(value_qualities)
                
                self.data[key] = DataLoader(CustomDatasetExpress(value, value_qualities), batch_size=self.cst["batch_size"],
                                            worker_init_fn=self.seed_worker,generator=g,shuffle=False)
                
            self.test_corr_dataloader_raw(key,expressive=expressive)
        print('------batches done')

    def getModel(self,config, modelKey=''):

        a = []
        if self.cst["task_name"]=="human":
            a = NVAE_LabReg(config, device=self.dev).to(self.dev)
    ...
    def training_loop_laban_qualities(self):
        ...
        for epoch in tqdm.tqdm(range(1, n_epochs + 1)):
            ...
            for _,seq_true in enumerate(tqdm.tqdm(self.data['train'])):
                lab_qualitie_true = seq_true[1].float().to(self.dev)
                seq_true = seq_true[0].to(self.dev)
                ...
                loss_mse_qualities = mse_loss(lab_qualitie_true,laban_output)
                loss = loss + self.cst["lambda_loss"]*loss_mse_qualities
            ...
            self.writer.add_scalar('MSE/MSE Loss Qualities Enc', loss_mse_qualities, epoch)
            ...
            print(f"MSE Loss Qualities Laban: {loss_mse_qualities}")
    ...
    def visualize_output(self, model,aa=None, train=False):
        ...
        if isinstance(trainDataTensor, np.ndarray) or isinstance(testDataTensor, np.ndarray):
            ...
            # print(trainDataTensor == testDataTensor)
        ...
        # testDataTensor = testDataTensor.permute(0,2,1)
        if self.cst["modelKey"]=="LVAE":
            predictions,latentSignal,_,_,_,_,laban_values= model(testDataTensor.float().to(self.dev))
            real_laban_values = torch.tensor(self.qualities_input_data["val"])
            print("Mean value for target Laban qualitie validation data:", torch.mean(real_laban_values,dim=0))
            print("Mean value for target Laban qualitie prediction data:", torch.mean(laban_values,dim=0))
        else:
        ...
        # trainDataTensor = trainDataTensor.permute(0,2,1)
        if self.cst["modelKey"]=="LVAE":
            predictions,latentSignal,_,_,_,_,laban_values= model(trainDataTensor.float().to(self.dev))
            real_laban_values = torch.tensor(self.qualities_input_data["train"])
            print("Mean value for target Laban qualitie train data:", torch.mean(real_laban_values,dim=0))
            print("Mean value for target Laban qualitie prediction data:", torch.mean(laban_values,dim=0))
        else:
        ...
        if not train:
            ...
            print(len(correlationCoeffArr2[correlationCoeffArr2<0.5]))
            # for c,ind in enumerate(correlationCoeffArr2[correlationCoeffArr2<0.5]):
            #     print(ind)
            #     n = np.where(correlationCoeffArr2 == ind)
            #     print(n[0])
            #     self.utils.visualizeData(n[0][0],trainDataTensor,predictions, title="Input Signals, Index: ")
        ...
        if self.cst["pretrained_model"]:

            self.general_dataset_study(correlationCoeffArr2, tag="Train Dataset")
            if self.cst["visualize_label_dist"]:
                self.plot_labels_dist(correlationCoeffArr2, tag="train")
            plt.show()

            self.general_dataset_study(correlationCoeffArr1, tag="Validation Dataset")
            if self.cst["visualize_label_dist"]:
                self.plot_labels_dist(correlationCoeffArr1, tag="val")
            plt.show()

        return correlationCoeffArr1, correlationCoeffArr2

    def general_dataset_study(self, correlation_array, tag="No tag"):
        sns.displot(correlation_array,kde=True, bins=20).set(title="Correlation Coefficient Distribution - "+ tag)

    def plot_labels_dist(self, correlationCoeff, tag="No tag"):
        _, bin_edges = np.histogram(correlationCoeff, bins=20)
        inds = np.digitize(correlationCoeff, bin_edges)
        aux_dict = {"correlation coefficient bin index":correlationCoeff}
        map = {"emo":"emotion", "act":"actor"}
        for key, value in self.dataset_tags[tag].items():
            aux_dict[map[key]] = value
        dataframe = pd.DataFrame(aux_dict)
        print(dataframe.head())
        plt.figure()
        sns.scatterplot(x="correlation coefficient bin index",
                         y="emotion", hue="actor", 
                         data=dataframe).set(title="Emotion and Actor vs Correlation Coefficient - "+ tag)
